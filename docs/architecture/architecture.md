...
**Решение:** ввести роль `create_markdown.registration_vision` (`provider` + `model` + `fallback`),
которая **синхронно** получает ту же vision-модель, что выбрана в секции «vision» настроек
(решение №9). Отдельного селектора «регистрация» нет: выбранная vision-модель пишется сразу в обе
роли `table_vision` и `registration_vision` (аналогично решению №6 для чат-модели).

**Обоснование:**
1. `table_vision` заточена под «таблица → markdown», а регистрация читает **титульную страницу**
   документа (обозначение + заголовок); промпты разные, но модель одна (одна vision-модель на обе
   задачи — проще для пользователя, решение №9).
2. Добавление ключа в секцию `roles` — **аддитивное изменение конфига**: пайплайны читают только
   известные им роли (`table_vision`, `ai_postprocess`, `query_processing`, `embedding`, `rerank`) и
   не сломаются от неизвестного ключа.

**Синхронная запись** (`config_ui.py`): при сохранении vision-модели веб атомарно пишет её одной
записью в `roles.create_markdown.table_vision` И `roles.create_markdown.registration_vision`
(primary + fallback одинаково), аналогично §4.3.

**Ограничение:** модель в `registration_vision` обязана иметь тег `vision` (в секции «vision» UI
показывает только vision-модели, требования §5.3).
