...
## Возобновление из кэша

Повторный запуск по тому же PDF/DOCX автоматически возобновляется из
кэшированных артефактов (`tmp/<stem>/`), без повторного вызова API и без
требования `YANDEX_API_KEY`/`YANDEX_FOLDER_ID`:

- `yandex_result.json` (валидный: непустой список страниц с `result.textAnnotation`)
  → данные Яндекса берутся из кэша, OCR не выполняется;
- `table_<idx>.md` (первая непустая строка — маркер `<!-- t_pN_M -->`) → таблица
  считается распознанной; недостающие `table_*.md` дозапускаются через vision
  только по недостающему подмножеству.

Локальные стадии (вырезка картинок/таблиц, постобработка, AI) перезапускаются
детерминированно, поэтому итоговый `.md` побайтово совпадает с полным прогоном.
Принудительный полный прогон: удалите `tmp/<stem>/yandex_result.json`
(и, при необходимости, `tmp/<stem>/table_*.md`).
