...
- **Многостраничная «Таблица 4.1» СП 52.13330.2016 не объединяется в одну таблицу (2026-08-15):** spatial-pass в `extract_table_images` (≈1842) требует нижнюю грань предыдущей таблицы ≥ 90% высоты страницы (hardcoded `page_height * 0.9`). У таблиц СП 52 низ — 79–86% (снизу колонтитул «СП 52.13330.2016» + номер страницы), поэтому `component_images` не формируется и `_merge_by_component_images` не склеивает. Итог: 4 страницы (14–17) → 2 отдельные таблицы («Таблица 4.1» разряды I–IV и «Продолжение таблицы 4.1» V–VIII). Данные не теряются. Порог калиброван на СП89/ГОСТ18410, где продолжения упираются в низ; нужна калибровка/адаптивный сигнал (решение — за architect).
...
- **Отложено (по решению пользователя, 2026-08-15):** перевести пайплайн на работу с Yandex JSON + исходным PDF, без промежуточного `raw.md` (см. Recent decisions). Снимает оба Known issues — склейка «Таблицы 4.1» и неснятые ID-маркеры. Реализация — architect (целевая модель данных + инкрементальный план миграции) → coder → reviewer; старт после освобождения пайплайна.
...
- **Пайплайн работает с Yandex JSON + исходным PDF, НЕ с raw.md (2026-08-15, решение пользователя):** структурированный источник — `textAnnotation` (`blocks`/`tables`/`pictures`/`fullText`) + PDF через `fitz` для вырезки рисунков/таблиц; рендер Markdown — только финальный выход. Промежуточный `raw.md` как источник истины для downstream-этапов устраняется. Следствие: ID-маркеры `<!-- t_pN_M -->` и вся их механика становятся не нужны — привязка JSON-таблицы ↔ vision-вырезки идёт по структуре `(page, table_index)`, а не через маркеры в тексте. Это снимает оба отложенных Known issues (склейка «Таблицы 4.1» + неснятые ID-маркеры). Реализация — architect (целевая модель данных + инкрементальный план миграции) → coder → reviewer; старт после освобождения пайплайна. Текстовые шаги (LaTeX-чистка, OCR-артефакты) по природе текстовые — куда их переносить (на поля JSON или на финальный MD) решает architect.
