...
_Last updated: 2026-08-15 — by: orchestrator — task: фикс ID-маркеров таблиц завершён (PASS), R-2 в очереди
...
- **R-2 (известное ограничение):** продолжение таблицы с подписью «Продолжение/Окончание таблицы N» (`has_caption=True`) spatial-pass не группирует → в `_assets.json` у склеенной таблицы `image_paths` только первая страница. На корпусе СП89/ГОСТ продолжения без подписей, не проявляется; планируется фикс (расширение spatial-pass на явные подписи продолжения).

## In progress

- Нет активных задач. Следующая — **R-2**: расширить spatial-pass `extract_table_images()` на подписи «Продолжение/Окончание таблицы N», чтобы в `component_images` попадали все страницы. Дизайн согласован, ждёт подтверждения пользователя на запуск.
