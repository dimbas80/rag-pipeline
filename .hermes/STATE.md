...
_Last updated: 2026-08-15 — by: orchestrator — диагностика завершена; фиксы (склейка таблиц + маркеры) отложены в backlog
...
- **Отложено (по решению пользователя, 2026-08-15):** исправить объединение многостраничных таблиц — заменить жёсткий порог 90% в spatial-pass на структурный/сквозной сигнал продолжения (см. Known issues «Таблица 4.1 СП52 не объединяется»), и детерминированно снимать оставшиеся ID-маркеры после AI-цикла (см. Known issues «AI-сверка оставляет маркеры»). Обе правки — через architect→coder→reviewer; стартовать после того, как пайплайн освободится от текущего документа.
