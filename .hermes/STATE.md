...
- **t_de63f7f5 (architect)** — диагностика 2 дефектов, найденных на живом прогоне СП 52.13330.2016 (см. Known issues ниже): (A) неполная склейка Таблицы 4.1 (4 страницы — склеены только парами 1-2 и 3-4, не в цепь); (B) AI-постобработка Таблицы 7.1 портит корректный результат (теряет строки, значение "M", ставит нестандартный LaTeX-делимитер).
