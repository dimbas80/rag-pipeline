...
def test_vision_auth_and_request_errors_go_to_fallback_without_retry(status_code):
    ...
    with patch.object(pipeline.httpx, "Client", return_value=client), patch.object(pipeline.time, "sleep") as sleep, patch.dict(os.environ, {"FB_KEY": "key"}):
        result = pipeline._call_vision_api("img", "prompt", config, "primary-key")
...
def test_ai_auth_and_request_errors_go_to_fallback_without_retry(status_code):
    ...
    with patch.object(pipeline.httpx, "Client", return_value=client), patch.object(pipeline.time, "sleep") as sleep, patch.dict(os.environ, env, clear=False):
        result = pipeline._call_ai_api("text", config)
...
def test_ai_recognition_failure_goes_to_fallback_without_retry(content):
    ...
    with patch.object(pipeline.httpx, "Client", return_value=client), patch.object(pipeline.time, "sleep") as sleep, patch.dict(os.environ, {"DEEPSEEK_API_KEY": "primary-key", "FB_KEY": "fallback-key"}, clear=False):
        result = pipeline._call_ai_api("text", config)
...
def test_recognition_failure_does_not_reject_valid_markdown_table():
    assert pipeline._is_recognition_failure("| A | B |\n|---|---|\n| 1 | 2 |") is False


def test_recognition_failure_detects_english_unable_phrase():
    assert pipeline._is_recognition_failure("Sorry, I am unable to read this image") is True
...
def test_ai_503_retries_then_returns_success():
    ...
    with patch.object(pipeline.httpx, "Client", return_value=client), patch.object(pipeline.time, "sleep") as sleep, patch.dict(os.environ, {"DEEPSEEK_API_KEY": "primary-key"}, clear=False):
        result = pipeline._call_ai_api("text", config)
...
def test_ai_none_content_is_handled_as_recognition_failure():
    ...
    with patch.object(pipeline.httpx, "Client", return_value=client), patch.dict(os.environ, {"DEEPSEEK_API_KEY": "primary-key", "FB_KEY": "fallback-key"}, clear=False):
        result = pipeline._call_ai_api("text", {"prompt": "process", "fallback": {"base_url": "https://fallback", "api_key_env": "FB_KEY"}})
    assert result == "fallback"